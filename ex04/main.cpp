...
void	replace_str(std::string filename, std::string s1, std::string s2)
{
	...
	{
		std::cout << std::endl
			<< "Error, can't have an empty search string, bucko!"
			<< std::endl;
		return;
	}
	...
	{
		isEmpty = false;
		size_t s1_position = 0;

		while ((s1_position = line.find(s1, s1_position))
			!= std::string::npos)
		{
			line.erase(s1_position, s1_length);
			line.insert(s1_position, s2);
			s1_position += s2.length();
		}
	...
	if (isEmpty)
		std::cout << "The selected file is empty!" << std::endl;
}
